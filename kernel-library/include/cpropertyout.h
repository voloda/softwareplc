 ...
 */
...
#ifndef _RTL_PLC_CPROPERTYOUT_H
	#define _RTL_PLC_CPROPERTYOUT_H
	 ...
	 */
	template<class DataType> class CPropertyOut
	{
		private:
			/**
		...
		public:
			 ...
			 */
			CPropertyOut(CVariable<DataType> *Variable = NULL)
			{
			...
			{
				return((*m_pStoredData));
			};

			/**
			 * Copying constructor for class
			 */
			CPropertyOut<DataType>& operator =(const CPropertyOut<DataType> &Property)
			{
				if (this != &Property)
				{
